...
// Create a fresh, empty vector
template <class T> void Vector<T>::create() { head = tail = 0; }

// Set up using a pair of iterators
template <class T> void Vector<T>::create(const T *b, const T *e) {
...
template <class T> T &Vector<T>::operator[](size_t i) { return at(i); }
